...
function find-regex-in-dart-test-for-local() {
  local pattern="$1"
  local dir="${2:-.}"
...
function _validate_git_repo() {
  ...
  git show-ref --verify --quiet "refs/heads/$base" || { echo "❌ Base branch '$base' not found"; return 1; }
  git show-ref --verify --quiet "refs/heads/$current" || { echo "❌ Current branch '$current' not found"; return 1; }
  return 0
}
...
function find-regex-in-dart-test-for-git() {
